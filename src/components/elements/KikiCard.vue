<template>
	...
	>
		...
		<div class="kiki-card__back">
			...
			<DinoButton v-if="!audio" class="kiki-card__button"
				text="Play Kiki"
				@click.native="playKiki"
			/>
			<DinoButton v-else
				class="kiki-card__button"
				text="Pause Kiki"
				@click.native="pauseKiki"
			/>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		playKiki: function () {
			this.audio = new Audio(require('../../../public/audios/' + this.data.file_name + '.mp3'))
