...
function loadDino() {
    ...
    dinoOpen.src = "images/dinoa-bday.png";
    dinoClosed.src = "images/dinof-bday.png";
}
    ...
    $('.kiki-form').on('submit', function (e) {
       ...
       if ((file_ext != "mp3") && (file_ext != "m4a") && (file_ext != "wav")){
        ...
        if (proceed) {        
            ...
            post_data.append( 'name', user_name);
            post_data.append( 'email', user_email);
            post_data.append( 'kiki-name', user_kikiName);
            post_data.append( 'twitter', user_twitter);
            post_data.append( 'message', user_message);
            post_data.append( 'kiki-file',  $('#cKiki')[0].files[0]);
            $.ajax({
                url: 'form.php',
                data: post_data,
                processData: false,
                ...
                success:function(result) {
                    ...
                    $('#cKiki').val('');
                    $('#cMessage').val('');
                    $('.success-form').html("Thanks for the kiki!");
                },
                error:function(result) {
                    $('.error-form').html("oops! please try again!");
                }
            })
        }    
    });
